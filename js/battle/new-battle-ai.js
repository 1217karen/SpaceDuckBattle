...
//敵をよける
export function isDangerCell(unit, units, x, y) {
...
//heal
  if (unit.role === "heal") {
    return units.some(u =>
      u.hp > 0 &&
      u.team !== unit.team &&
      Math.max(Math.abs(u.x - x), Math.abs(u.y - y)) <= 1
    );
  }
...
//support
  if (unit.role === "support") {
    return units.some(u =>
      u.hp > 0 &&
      u.team !== unit.team &&
      Math.abs(u.x - x) + Math.abs(u.y - y) <= 1
    );
  }
...
//スコアを見る
export function createScoreFn(unit) {

  return ({ units, x, y, moveMode, newDist }) => {

    let score = (moveMode === "toward") ? -newDist : newDist;
//support
    if (unit.role === "support") {
