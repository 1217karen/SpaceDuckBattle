...
import { createMapRoomSectionController } from "./map-room-section.js";
...
import { getPublicRoomsByParentAreaId } from "../services/room-service.js";
...
const mapRoomSectionController = createMapRoomSectionController({
  getCurrentAccount,
  findPlaceById,
...
window.addEventListener("beforeunload", event => {
  if (!mapRoomSectionController.hasUnsavedRoomChanges()) {
    return;
  }
...
function moveToPlaceWithConfirmation(placeId) {
  if (!mapRoomSectionController.confirmDiscardRoomChanges()) {
    return;
  }

  mapRoomSectionController.clearRoomFormTracking();
  moveToPlace(placeId);
}
...
function renderMapTree() {
  ...
  if (!selectedFieldId) {
    ...
    mapContent.appendChild(mapRoomSectionController.renderRoomCreatorSection(currentPlaceId));
